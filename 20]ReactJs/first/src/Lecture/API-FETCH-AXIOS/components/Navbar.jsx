...
const Navbar = () => {
  return (
    <nav className="fixed top-4 left-1/2 transform -translate-x-1/2 flex gap-8 px-10 py-3 rounded-full bg-white/20 backdrop-blur-lg shadow-lg z-50">
        ...
        {/* This NavLink navigates to the Show route ('/Show'). It applies a dynamic className based on its `isActive` status. */}
        <NavLink 
          to="/Show"
        ...
        {/* This NavLink navigates to the Service route ('/Service'). It applies a dynamic className based on its `isActive` status. */}
        <NavLink 
          to="/Service"
          className={({ isActive })=>`font-medium text-lg transition hover:-translate-y-0.5 ${isActive ? "text-black font-semibold" : "text-grey"} `}
        >
